...
def compute_train_IDF():
    ...
    logger.info("IDF weight calculate complete with training set!")
    return node_IDF, file_list

def compute_date_IDF(date):
    node_IDF = {}

    file_list = []
    file_path = artifact_dir + date + "/"
    file_l = os.listdir(file_path)
    for i in file_l:
        file_list.append(file_path + i)
    ...
    torch.save(node_IDF, artifact_dir + f"node_IDF_{date}")
    logger.info(f"IDF weight calculate complete with {date}!")
    return node_IDF, file_list
...
# This is for April 10th and 11th
def cal_set_rel(s1, s2, node_IDF, tw_list):
    ...
    for i in new_s:
        if is_include_key_word(i) is not True:
            ...
            if IDF > math.log(len(tw_list) * 0.9 / (1)):
                logger.info("node:", i, " IDF:", IDF)
                count += 1
...
# This is for April 12th
def cal_set_rel_12(s1, s2, node_IDF, tw_list, date_IDF, date_tw_list):
    def is_include_key_word(s):
        # The following common nodes don't exist in the training/validation data, but
        # will have the influences to the construction of anomalous queue (i.e. noise).
        # These nodes frequently exist in the testing data but don't contribute much to
    ...
    for i in new_s:
        if is_include_key_word(i) is not True:
            if i in date_IDF.keys():
                IDF = date_IDF[i]
            else:
                IDF = math.log(len(date_tw_list) / (1))

            if i in node_IDF.keys():
                IDF3 = node_IDF[i]
            else:
                IDF3 = math.log(len(tw_list) / (1))
...
# This is for April 12th
def anomalous_queue_construction_12(node_IDF, tw_list, date_IDF, date_tw_list, graph_dir_path):
    history_list = []
    current_tw = {}
    ...
    for f_path in sorted(file_l):
        ...
        for hq in history_list:
            for his_tw in hq:
                if cal_set_rel_12(current_tw['nodeset'], his_tw['nodeset'], node_IDF, tw_list, date_IDF, date_tw_list) != 0 and current_tw['name'] != his_tw['name']:
                    hq.append(copy.deepcopy(current_tw))
                    added_que_flag = True
                    break
                if added_que_flag:
...
if __name__ == "__main__":
    ...
    node_IDF, tw_list = compute_train_IDF()
    date_IDF, date_tw_list = compute_date_IDF('graph_4_12')
    ...
    history_list = anomalous_queue_construction_12(
        node_IDF=node_IDF,
        tw_list=tw_list,
        date_IDF=date_IDF,
        date_tw_list=date_tw_list,
